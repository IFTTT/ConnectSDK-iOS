//
//  ConnectionCredentials+TokenRequest.swift
//  Grocery Express
//
...
extension ConnectionCredentials {
    
    /// Build a token request for these `ConnectionCredentials`
    func tokenRequest() -> Request {
        return Request(credentials: self)
    }
    ...
    struct Request {
        ...
        func start(_ completion: ((ConnectionCredentials) -> Void)?) {
        ...
        init(credentials: ConnectionCredentials) {
            ...
            self.urlRequest = credentials.getTokenURLRequest()
        }
    }
    ...
    private func getTokenURLRequest() -> URLRequest {
        ...
        components.queryItems = [URLQueryItem(name: "code", value: partnerOAuthCode)]
